...
class Proba:
    ...
    def crossover_diff(self, ind, mutant, cross_proba):
    ...
    def mutate_diff(self, xr1, xr2, xr3, F):
    ...
    def get_proba(self, pop, low_bound, up_bound):
        list_proba = []
        mini = 0
        maxi = len(pop)

        for col in range(len(pop[0])):
            val = sum(ind[col] for ind in pop)
            proba = ((val-mini)/(maxi-mini))*(up_bound-low_bound)+low_bound
            list_proba.append(proba)

        return list_proba

    def mutate_proba(self, list_proba):
        return np.random.rand(len(list_proba)) <= list_proba

    def write_res(self, folderName, name, mode, n_pop, n_gen, cross_proba, F, low_bound, up_bound, y1, y2, yX, colMax,
                  bestScorePro, bestAPro, bestPPro, bestRPro, bestFPro, bestModelPro, bestScore, bestScoreA,
                  bestScoreP, bestScoreR, bestScoreF, bestModel, debut, out, yTps):
        a = os.path.join(os.path.join(self.path2, folderName), 'resultat.txt')
        f = open(a, "w")
        string = "heuristique: Evolution probabilistique" + os.linesep + "mode: " + mode + os.linesep +\
                 ...
                 "borne inf: " + str(low_bound) + os.linesep + "borne sup: " + str(up_bound) + os.linesep +\
    ...
    def natural_selection(self, part, n_pop, n_gen, cross_proba, F, low_bound, up_bound, data, dummiesList,
        ...
        for mode in part:
            ...
            for generation in range(n_gen):
                ...
                diff_size = int(n_pop/2)
                proba_size = n_pop - diff_size
                ...
                for i in range(diff_size):
                    ...
                    # mutation
                    mutant = self.mutate_diff(xr1, xr2, xr3, F)

                    # croisement
                    trial = self.crossover_diff(pop[i], mutant, cross_proba)

                    mutants.append(trial)

                # Calcul les probas en fonction du nombre de fois qu'une variable est sélectionnée
                list_proba = self.get_proba(pop, low_bound, up_bound)
                
                for i in range(proba_size):

                    # mutant obtenu en fonction des probas
                    mutant = self.mutate_proba(list_proba)
                ...
                self.write_res(folderName=folderName, name=self.data_name, mode=mode, n_pop=n_pop, n_gen=n_gen,
                               cross_proba=cross_proba, F=F, low_bound=low_bound, up_bound=up_bound, y1=y1, y2=y2,
                               yX=yX, colMax=colMax, bestScorePro=bestScorePro, bestAPro=bestAPro, bestPPro=bestPPro,
                               ...
                               bestScoreF=bestScoreF, bestModel=bestModel, debut=debut, out=print_out, yTps=yTps)
    ...
    def init(self, n_pop, n_gen, cross_proba, F, low_bound, up_bound, data, dummiesList, createDummies, normalize,
             metric):

        print("########################################")
        print("#ALGORITHME A EVOLUTION PROBABILISTIQUE#")
        print("########################################")
        print()
        ...
        for part in mods:
            process = multiprocessing.Process(target=self.natural_selection,
                                              args=(part, n_pop, n_gen, cross_proba, F, low_bound, up_bound,
        ...
        return utility.res(heuristic="Evolution probabilistique", x=x, y=y, z=z,
...
if __name__ == '__main__':
    ...
    genetic = Proba(d2, d, ['lr'], target, originLst, name)
    pop = 5
    gen = 5
    cross_proba = 0.5
    F = 1
    low_bound = 0.1
    up_bound = 0.9
    g1, g2, g3, g4, g5 = genetic.init(n_pop=pop, n_gen=gen, cross_proba=cross_proba, F=F, low_bound=low_bound,
                                      up_bound=up_bound, data=copy2, dummiesList=d.dummiesList,
